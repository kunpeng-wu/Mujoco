...
from robot_hand.base_controller import Controller
...
class OperationalSpaceController(Controller):
    ...
    of the robot's end effector. For detailed information as to the mathematical foundation for this manipulator, please
    ...
    end effector or absolute values. In either case, a given action to this manipulator is assumed to be of the form:
    (x, y, z, ax, ay, az) if controlling pos and ori or simply (x, y, z) if only controlling pos

    Args:
        sim (MjSim): Simulator instance this manipulator will pull robot state updates from
        ...
        input_max (float or Iterable of float): Maximum above which an inputted action will be clipped. Can be either be
            ...
            latter, dimension should be the same as the control dimension for this manipulator
        ...
        output_max (float or Iterable of float): Maximum which defines upper end of scaling range when scaling an input
            ...
            each dimension). If the latter, dimension should be the same as the control dimension for this manipulator
        ...
        impedance_mode (str): Impedance mode with which to run this manipulator. Options are {"fixed", "variable",
            "variable_kp"}. If "fixed", the manipulator will have fixed kp and damping_ratio values as specified by the
            @kp and @damping_ratio arguments. If "variable", both kp and damping_ratio will now be part of the
            manipulator action space, resulting in a total action space of (6 or 3) + 6 * 2. If "variable_kp", only kp
        ...
        kp_limits (2-list of float or 2-list of Iterable of floats): Only applicable if @impedance_mode is set to either
            "variable" or "variable_kp". This sets the corresponding min / max ranges of the manipulator action space
        ...
        damping_ratio_limits (2-list of float or 2-list of Iterable of floats): Only applicable if @impedance_mode is
            set to "variable". This sets the corresponding min / max ranges of the manipulator action space for the
        ...
        policy_freq (int): Frequency at which actions from the robot policy are fed into this manipulator
        ...
        **kwargs: Does nothing; placeholder to "sink" any additional arguments so that instantiating this manipulator
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        assert impedance_mode in IMPEDANCE_MODES, (
            "Error: Tried to instantiate OSC manipulator for unsupported "
    ...
    def control_limits(self):
        """
        Returns the limits over this manipulator's action space, overrides the superclass property
