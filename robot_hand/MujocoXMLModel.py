...
class MujocoXML:
    def __init__(self, fname):
        ...
        self._replace_defaults_inline(default_dic=default_classes)  # Expand the default classes where it is called
        self.root.remove(default)   # Remove original default classes
...
class MujocoXMLModel(MujocoXML, MujocoModel):
    ...
    def __init__(self, fname, idn=0):
        ...
        self._elements = sort_elements(root=self.root, element_filter=_add_default_name_filter) # actuators/root_body/visual_geoms/contact_geoms/bodies/joints
        # print(self._elements["root_body"][0].get("name"))
        # print(self._elements["root_body"][1].get("name"))
        assert (
...
if __name__ == '__main__':
    model = MujocoXMLModel('../model/robot_hand/body_new.xml')
    # xml_str = model.get_xml()
    print(model.name)
    print(model.base_offset)
    print(model.bodies)
    print(model.joints)
    print(model.actuators)
    print(model.sites)
    print(model.sensors)

    hand_element = find_elements(
        root=model.root, tags="body", attribs={"name": "left_hand"}, return_first=True
    )
    hand_rotation_offset = string_to_array(hand_element.get("quat", "1 0 0 0"))[[1, 2, 3, 0]]
    print(hand_rotation_offset)

    hand_element = find_elements(
        root=model.root, tags="body", attribs={"name": "right_hand"}, return_first=True
    )
    hand_rotation_offset = string_to_array(hand_element.get("quat", "1 0 0 0"))[[1, 2, 3, 0]]
    print(hand_rotation_offset)

    cameras = model.get_element_names(model.worldbody, "camera")
    print(cameras)